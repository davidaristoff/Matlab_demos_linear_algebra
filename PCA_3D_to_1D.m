...
[u,lam] = eigs((1/n)*Xc'*Xc,3,'largestabs');  %u = 1st principal component
Xp = (Xc*u(:,1))*u(:,1)';               %Xp = projected centered data
Xr = Xm + Xp;                           %Xr = 1-term reconstructed data set

disp('variances along principal axes (i.e., eigenvalues of X^TX/n) = ...')
diag(lam)
...
subplot(2,2,4)
plot(Xc*u(:,1),zeros(n,1),'.','color',[0 .5 0],'markersize',15)
hold on
plot(0,0,'sr','linewidth',5)
axis([-1.5 1.5 -1.5 1.5])
legend('first principal coordinate','interpreter','latex','fontsize',14)
 title('1D PCA plot','interpreter','latex','fontsize',14)
 axis square
 set(gca,'fontsize',14)
